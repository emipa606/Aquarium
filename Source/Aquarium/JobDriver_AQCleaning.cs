...
namespace Aquarium
{
	...
	public class JobDriver_AQCleaning : JobDriver
	{
		...
		protected CompAquarium AQComp
		{
			get
			{
				return CleanThing.TryGetComp<CompAquarium>();
			}
		}
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			yield return Toils_General.Wait(AQUtility.GetCleanTime(AQComp), TargetIndex.None).FailOnDestroyedNullOrForbidden(TargetIndex.A).FailOnCannotTouch(TargetIndex.A, PathEndMode.Touch).WithProgressBarToilDelay(TargetIndex.A, false, -0.5f).WithEffect(EffecterDefOf.Clean, TargetIndex.A).PlaySustainerOrSound(() => SoundDefOf.Interact_CleanFilth);
